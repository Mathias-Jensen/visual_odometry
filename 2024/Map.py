...
class Map():
    ...
    def clean(self):
        ...
        self.observations = []
        self.camera_matrix: np.ndarray = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    ...
    def add_point_from_match(self, match):
        tp = TrackedPoint(
                match.point, 
                match.descriptor1, 
                match.color, 
                match.featureid1)
        return self.add_point(tp)
    ...
    def remove_observations_with_reprojection_errors_above_threshold(self, threshold = 100):
        ...
        for observation in self.observations:
            ...
            point_1 = np.array(point.point)
            point_1 = np.hstack((point_1, 1))
            point_1 = np.array([point_1]).T
            point_in_cam_coords = camera.pose() @ point_1
    ...
    def calculate_reprojection_error(self, threshold = 10):
        ...
        for observation in self.observations:
            ...
            point_2 = np.array(point_1.point)
            point_2 = np.hstack((point_2, 1))
            point_2 = np.array([point_2]).T
            point_in_cam_coords = camera.pose() @ point_2
            t = self.camera_matrix @ point_in_cam_coords[0:3, :]
            t = t / t[2, 0]
            dx = t[0] - observation.image_coordinates[0]
            dy = t[1] - observation.image_coordinates[1]
    ...
    def remove_camera_from_map(self, camera_to_remove):
        ...
        point_observations: dict[int, int] = collections.defaultdict(lambda: 0)
        for observation in self.observations:
    ...
    def show_number_of_observations_per_point(self):
        observations_per_point: dict[int, int] = collections.defaultdict(lambda: 0)
        for observation in self.observations:
    ...
    def show_observation_matrix(self):
        ...
        # What is the type of point.point_id, I expected it to be (feature_number, image_number)
        feature_id_distribution = collections.defaultdict(lambda: 0)
        for point in self.points:
            image_id = point.feature_id[0]
            val = feature_id_distribution[image_id]
            feature_id_distribution[image_id] = val + 1
        ic(feature_id_distribution)
