...
class VisualSlam:
    ...
    def initialize_map(self, image_pair):
        ...
        #print("Map statistics before optimization")
    ...
    def match_current_and_previous_frame(self):
        ...
        self.map.limit_number_of_camera_in_map(36)
        cv2.waitKey(100)
    ...
    def plot_camera_trajectory(self):
        """
        Plots the camera trajectory in 3D space.
        """
        camera_positions = [camera.t for camera in self.map.cameras]
        if not camera_positions:
            print("No camera positions available to plot.")
            return

        # Extract x, y, z coordinates
        x_coords = [pos[0]/0.3 for pos in camera_positions]
        y_coords = [pos[1]/0.3 for pos in camera_positions]
        z_coords = [pos[2] for pos in camera_positions]

        # Plot the trajectory
        plt.figure()
        plt.plot(x_coords, y_coords, marker='o', label='Camera Trajectory')
        plt.xlabel('X')
        plt.ylabel('Y')
